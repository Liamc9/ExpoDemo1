...
import { View, Text, ImageBackground, StyleSheet, Pressable, Dimensions, Alert, SafeAreaView } from "react-native";
...
async function signInWithApple() {
  try {
    ...
    if (!credential.identityToken) {
      Alert.alert("Apple Sign-In failed", "No identity token returned. Check Sign in with Apple capability and rebuild.");
      return;
    }
...
// ---------------- Screen ----------------
export default function SignInIOS() {
  ...
  return (
    <View style={s.root}>
      <ImageBackground source={require("../../assets/images/BakeryImage.jpg")} style={s.bg} resizeMode="cover">
        ...
        <LinearGradient colors={["transparent", "rgba(0,0,0,0.95)"]} start={{ x: 0.5, y: 0 }} end={{ x: 0.5, y: 1 }} locations={[0.2, 1]} style={s.gradient}>
          <View style={s.bottomBar}>
            <Pressable style={s.appleBtn} onPress={signInWithApple}>
              <Ionicons name="logo-apple" size={22} color="#fff" />
              <Text style={s.appleTxt}>Continue with Apple</Text>
            </Pressable>

            <Pressable style={s.googleBtn} onPress={() => promptAsync()} disabled={!request}>
